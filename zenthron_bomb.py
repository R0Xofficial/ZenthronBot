    ...
    if update.message.reply_to_message:
        ...
        if not target_entity:
            try:
                ...
                await update.message.reply_text(f"Error: Could not find user. Make sure you entered the details correctly.")
                return
    else:
...
async def unban_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        user_display_name = target_user.mention_html() or html.escape(getattr(target_user, 'full_name', '') or f"<code>{target_user.id}</code>")
